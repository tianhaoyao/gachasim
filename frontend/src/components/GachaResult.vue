                    ...
                    <div class="card-footer">
                        <div class="progress" v-for="(pity, rarity) in roll.pity" :key="rarity">
                            <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar"
                                :style="{ width: calculatePercent(pity, ratesInfo[rarity]?.pity), backgroundColor: getColor(pity, ratesInfo[rarity]?.softpity, ratesInfo[rarity]?.color) }"
                                :aria-valuenow="pity" aria-valuemin="0" :aria-valuemax="ratesInfo[rarity]?.pity">
                                {{ pity }}
                            </div>
                        </div>
                    </div>
